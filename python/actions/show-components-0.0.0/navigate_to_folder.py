...
import ftrack_api
import platform
from directory import open_directory

class NavigateToFolder(object):
    ...
    def resolve_path(self, component_id):
        ...
        if location:
            try:
                path = location.get_filesystem_path(component).replace(" ","")
...
def register(session, **kw):
    ...
    action = NavigateToFolder(session, logger)
    action.register()
